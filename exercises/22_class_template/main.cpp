...
struct Tensor4D {
    ...
    Tensor4D &operator+=(Tensor4D const &others) {
        // TODO: 实现单向广播的加法
        for (auto d = 0u; d < 4; ++d) {
            ASSERT(others.shape[d] == shape[d] || others.shape[d] == 1, "shape mismatch");
        }
            ...
            for (auto i1 = 0u; i1 < shape[1]; ++i1) {
                ...
                for (auto i2 = 0u; i2 < shape[2]; ++i2) {
                    ...
                    for (auto i3 = 0u; i3 < shape[3]; ++i3) {
                        ...
                        auto jdx = ((j0 * others.shape[1] + j1) * others.shape[2] + j2) * others.shape[3] + j3;
                        data[idx] += others.data[jdx];
                    }
                }
            }
        }
